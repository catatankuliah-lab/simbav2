...
class SJFebruariController extends ResourceController
{
    protected $model;
    protected $modelPBP;
    protected $modelLO;
    ...
    {
        $this->model = new FebruariSJModel();
        $this->modelPBP = new PBPFebruariModel();
        $this->modelLO = new LOFebruariModel();
    }
    ...
    public function uploadfile($idsj)
    {
        ...
        $alamatnya = FCPATH  . 'UPLOAD/2/SJ/' . $namagudang . '/' . $datasj->tanggal_muat . '/';
        $path = 'UPLOAD/2/SJ/' . $namagudang . '/' . $datasj->tanggal_muat . '/';
