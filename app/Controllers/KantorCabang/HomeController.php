...
class HomeController extends Controller
{
    ...
    protected $loJanuari;
    protected $januariSJ;
    protected $loFebruari;
    protected $februariSJ;

    public function __construct()
    {
        $this->loJanuari = new LOJanuariModel();
        $this->januariSJ = new JanuariSJModel();
        $this->loFebruari = new LOFebruariModel();
        $this->februariSJ = new FebruariSJModel();
    }
    ...
    public function detail_lo_februari($nomorlo)
    {
        $datalo = $this->loFebruari->detaillocabang($nomorlo);
        $data = [
            'menu1' => '',
            'menu2' => 'selected',
            'menu3' => '',
    ...
    public function generateLaporanwo_februari($nomorwo)
    {

        $data = $this->loFebruari->woPDF($nomorwo);
    ...
    public function generateReport($idalokasi)
    {
        ...
        $bahantanggal = $this->loJanuari->getSPMPerKecamtanTanggal($idKantor);
        // dd($bahan);
        $excel = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
        $no = 1;

        foreach ($bahantanggal as $a) {

            $bahan = $this->loJanuari->getSPMPerKecamtan($a->id_kantor, $a->tanggal_muat);
            ...
            foreach ($bahan as $b) {
                ...
                if ($b != null) {
                    ...
                    $no++;
                    $sheet->setTitle($b->nama_gudang . ' ' . $b->tanggal_muat, true);
                    ...
                    $sheet->setCellValue('C5', ': ' . $b->tanggal_muat);
                    ...
                    $tanggal = $b->tanggal_muat;
                    $gudang = $this->loJanuari->getRekap($idGudang, $tanggal);
                    // dd($gudang);
                    foreach ($gudang as $row) {
                        ...
                        $sheet->setCellValue('H' . $mulai, $row->nomor_danom);
    ...
    public function generateReportFebruari($tanggal)
    {

        if ($tanggal == "1") {
            $idKantor = session()->get('id_kantor_cabang');
            $bahantanggal = $this->loFebruari->getSPMPerKecamtanTanggal($idKantor);
            // dd($bahan);
            $excel = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
            $no = 1;
            ...
            foreach ($bahantanggal as $a) {
                ...
                $bahan = $this->loFebruari->getSPMPerKecamtan($a->id_kantor, $a->tanggal_muat);
                ...
                foreach ($bahan as $b) {
                    ...
                    if ($b != null) {
                        ...
                        $no++;
                        $sheet->setTitle($b->nama_gudang . ' ' . $b->tanggal_muat, true);
                        $sheet->setCellValue("A1", "REKAPITULASI HARIAN BANTUAN PANGAN CADANGAN BERAS PEMERINTAH 2024 FEBRUARI");
                        $sheet->mergeCells('A1:J1');
                        $alignment = $sheet->getStyle('A1:J1')->getAlignment();
                        $alignment->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                        $boldStyle = $sheet->getStyle('A1:J1')->getFont();
                        ...
                        $idGudang = $b->id_gudang;
                        $tanggal = $b->tanggal_muat;
                        $gudang = $this->loFebruari->getRekap($idGudang, $tanggal);
                        // dd($gudang);
                        foreach ($gudang as $row) {
                            $total = $total + ($row->jumlah_penyaluran_januari / 10);
                            $sheet->setCellValue('A' . $mulai, $no);
            ...
            $bahan = $this->loFebruari->getSPMPerKecamtan(session()->get('id_kantor_cabang'), $tanggal);
    ...
    public function downloadwo1($kodewo)
    {
        // Inisialisasi TCPDF
        $data = $this->loJanuari->findbykodewo($kodewo);
        // dd($data);
        $pdf = new FpdiTcpdf();
        $pdf->setPrintHeader(false);
        $pdf->setPrintFooter(false);
        $pdf1Path = $data[0]->path_upload_wo . $data[0]->file_upload_wo;
        $pdf->setSourceFile($pdf1Path);
        for ($i = 1; $i <= $pdf->setSourceFile($pdf1Path); $i++) {
            $tplIdx = $pdf->importPage($i);
            $pdf->AddPage();
            $pdf->useTemplate($tplIdx);
        }
        foreach ($data as $bahandokumen) {
            $pdf2Path = $bahandokumen->path_lo . $bahandokumen->file_uplaod_lo;
            $pdf->setSourceFile($pdf2Path);
            for ($i = 1; $i <= $pdf->setSourceFile($pdf2Path); $i++) {
                $tplIdx = $pdf->importPage($i);
                $pdf->AddPage();
                $pdf->useTemplate($tplIdx);
            }
            $pdf3Path = $bahandokumen->path_uplaod_do . $bahandokumen->file_upload_do;
            $pdf->setSourceFile($pdf3Path);
            for ($i = 1; $i <= $pdf->setSourceFile($pdf3Path); $i++) {
                $tplIdx = $pdf->importPage($i);
                $pdf->AddPage();
                $pdf->useTemplate($tplIdx);
            }
        ...
        // Simpan atau keluarkan PDF sebagai unduhan
